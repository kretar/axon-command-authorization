...
public class CommandAuthorizationInterceptor implements MessageHandlerInterceptor<CommandMessage<?>> {
    ...
    @Override
    public Object handle(UnitOfWork<? extends CommandMessage<?>> unitOfWork, @javax.annotation.Nonnull InterceptorChain interceptorChain) throws Exception {
        ...
        Set<GrantedAuthority> userId = Optional.ofNullable(command.getMetaData().get("authorities"))
                .map(uId -> {
                    ...
                    return Set.copyOf((List<GrantedAuthority>) uId);
                })
                .orElseThrow(() -> new UnauthorizedCommandException("No authorities found"));
