...
class CommandAuthorizationInterceptorTest {
    ...
    public void shouldAuthorizeAndPropagateUsername() {
        ...
        fixture.registerCommandDispatchInterceptor(new ReactorAuthorizationMessageDispatchInterceptor())
                .registerCommandHandlerInterceptor(new CommandAuthorizationInterceptor())
                .registerCommandHandlerInterceptor(new CorrelationDataInterceptor<>(new SimpleCorrelationDataProvider("username")))
                .given()
                .when(new CreateAggregateCommand(aggregateId))
                .expectSuccessfulHandlerExecution()
                .expectResultMessageMatching(Matchers.matches(message -> ((User)message.getMetaData().get("username")).getUsername().equals("admin")));
    }
